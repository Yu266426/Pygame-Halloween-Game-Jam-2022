...
from data.modules.annoyances.spider import HangingSpider
from data.modules.annoyances.web import Web
from data.modules.base.constants import TILE_SIZE
from data.modules.base.inputs import InputManager
from data.modules.base.loader import Loader
from data.modules.game.tile import Grass
from data.modules.base.utils import man_dist
from data.modules.game.walls import HorizontalWall, VerticalWall
...
class Maze:
	def __init__(self, size: tuple[int, int]):
		...
		self.obstacles: list[list[None | Web]] = [[None for _ in range(self.size[0])] for __ in range(self.size[1])]
		self._scatter_obstacles(100)
	...
	def _scatter_obstacles(self, n):
		self.obstacles: list[list] = [[None for _ in range(self.size[0])] for __ in range(self.size[1])]

		for _ in range(n):
			x = random.randrange(0, self.size[0])
			y = random.randrange(0, self.size[1])

			self.obstacles[y][x] = Web((x * TILE_SIZE, y * TILE_SIZE))
	...
	def update(self, player_tile_pos):
		# if InputManager.keys_down[pygame.K_SPACE]:
		# 	self.regen()

		if player_tile_pos != self.prev_player_pos:
			if self.obstacles[player_tile_pos[1]][player_tile_pos[0]]:
				for _ in range(random.randint(0, 2)):
					Annoyances.add_annoyance(HangingSpider())
			self.prev_player_pos = player_tile_pos
	...
	def draw(self, surface: pygame.Surface, camera: pygame.Vector2):
		...
		surface.blit(Loader.images["end"], ((self.size[0] - 1) * TILE_SIZE - camera.x, (self.size[1] - 1) * TILE_SIZE - camera.y))

		for wall in self.walls:
			wall.draw(surface, camera)

	def foreground_draw(self, surface: pygame.Surface, camera: pygame.Vector2):
